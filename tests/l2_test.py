...
def test_batchnorm_l2_layer():
    ...
    # Initialize BatchNormLayer and L2RegularizationLayer
    batch_norm_layer = BatchNormLayer(num_features=10)
    l2_layer = L2RegularizationLayer(layer=batch_norm_layer, lambda_=0.01)

    # Forward pass
    outputs = l2_layer.forward(inputs)
    ...
    # Backward pass
    grads = l2_layer.backward(dL_dout, lr)
    ...
    assert not np.allclose(updated_gamma, np.zeros((1, 10))), "Gamma not updated"
    assert not np.allclose(updated_beta, np.zeros((1, 10))), "Beta not updated"

    print("All tests passed.")

# Run the test
test_batchnorm_l2_layer()
